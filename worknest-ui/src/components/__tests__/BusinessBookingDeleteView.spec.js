...
describe("BookingWorkStationDelete", () => {
    ...
    beforeEach(() => {
        wrapper = shallowMount(BookingWorkStationDelete, {
            global: {
                mocks: {
                    $ApiService: {
                        find_user_by_id: vi.fn(() => Promise.resolve({ data: { companyId: "company-id" } })),
                        get_booking_businesses_by_user_id: vi.fn(() => Promise.resolve({ data: [] })),
                        get_bookings_by_business_booking_id: vi.fn(() => Promise.resolve({ data: [] })),
                        delete_booking: vi.fn(() => Promise.resolve("Booking deleted successfully")),
                    },
                },
            },
        });
    ...
    it("deleteItem should call delete_booking API method and update bookings", async () => {
        ...
        expect(wrapper.vm.$ApiService.delete_booking).toHaveBeenCalledWith("booking-id");
    });
    ...
    it("getStatusColor should return the correct color based on status and date", () => {
        const activeBooking = { status: "active", startDate: "2024-04-07T09:00:00Z" };
        const canceledBooking = { status: "canceled", startDate: "2024-04-07T09:00:00Z" };
