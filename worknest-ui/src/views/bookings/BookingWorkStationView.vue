    ...
    <v-row justify="center" v-show="!isSvgVisible" class="mx-auto px-12">
      <v-col cols="12" md="8" lg="6">
        <v-card class="mb-4 d-block rounded-lg" elevation="2">
          <v-row>
            <v-col>
              <v-date-picker v-model="booking.startDate" :min="new Date(new Date().setDate(new Date().getDate() - 1))"
                no-title scrollable class="ml-8"></v-date-picker>
            </v-col>
            <v-col>
              <p class="text-h4 mt-12 pt-5" style="font-size: 32px !important">Select building</p>
              <v-select class="my-8 mr-5" label="Select" chips variant="outlined" :items="buildings" item-title="name"
                v-model="selectedBuilding"
                :hint="selectedBuilding !== null ? (selectedBuilding.address + ' ' + selectedBuilding.streetNumber + ', ' + selectedBuilding.city + ' (' + selectedBuilding.province + ')') : ''"
  ...
  <v-row scrollable>
          ...
          <v-card elevation="5" class>
            ...
            <v-card-text>
              <v-list>
                ...
                <v-list-item v-if="deskDetails.equipment && deskDetails.equipment !== 'null'">
                  <v-list-item-content>
    ...
    <v-fade-transition hide-on-leave>
      <v-dialog v-model="alertVisible" persistent min-width="500" max-width="800">
        <v-card class="mx-auto booking-alert" elevation="16" min-width="500" max-width="800" :title="alertType === 'success' ? 'Confirmed booking' : 'Error booking'
          ">
          <v-divider></v-divider>

          <div class="py-12 text-center">
            <v-icon class="mb-6" size="128" :color="alertType === 'success' ? 'success' : 'error'">
              {{
                alertType === "success"
