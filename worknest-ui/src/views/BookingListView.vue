  ...
  >
    ...
    <template v-slot:item.actions="{ item }">
      <v-icon
        ...
        size="small"
        @click="editItem(item)"
        :class="{disabled:item.status !== 'canceled' && new Date(new Date().setHours(0,0,0,0)) <= new Date(new Date(item.startDate).setHours(0,0,0,0))  ? false : true}"
      >
        mdi-pencil
      </v-icon>
      <v-icon
    ...
    <template v-slot:item.startDate="{item}">
      <v-icon
        class="me-2"
        size="small"
        :color="item.status == 'canceled' ? 'red' : (item.status == 'active' && new Date(new Date().setHours(0,0,0,0)) > new Date(new Date(item.startDate).setHours(0,0,0,0))) ? 'gray' : 'green'"
      >
        {{item.status == 'canceled' ? 'mdi-calendar-remove' : (item.status == 'active' && new Date(new Date().setHours(0,0,0,0)) > new Date(new Date(item.startDate).setHours(0,0,0,0))) ? 'mdi-calendar-check' : 'mdi-calendar-clock'}}
      </v-icon>
      {{item.startDate}}
    </template>
    <template v-slot:item.status="{ item }">
    <v-chip :color="item.status == 'canceled' ? 'red' : (item.status == 'active' && new Date(new Date().setHours(0,0,0,0)) > new Date(new Date(item.startDate).setHours(0,0,0,0))) ? 'gray' : 'green'">
      {{ item.status }}
    </v-chip>
  </template>
