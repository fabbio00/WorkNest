...
<template>
  <div class="text-center my-7">
    <img src="/worknest-logo.ico" class="d-inline-flex" style="max-height: 100px" align="center" />
    <p v-if="!bookingId" class="text-h3 d-inline font-italic ml-5" style="vertical-align: middle">
      Book desks for your team
    </p>
    <p v-else class="text-h3 d-inline font-italic ml-5" style="vertical-align: middle">
      Modify your booking
    </p>
  </div>
  <Transition enter-active-class="animate__animated animate__fadeIn"
    enter-leave-class="animate__animated animate__fadeOut" appear>
    <v-row justify="center" v-show="!isSvgVisible" class="mx-auto px-12">
      <v-col cols="12" md="8" lg="6">
        <v-card class="mb-4 d-block rounded-lg" elevation="2">
          <v-row>
            <v-col>
              <v-date-picker v-model="this.selectedDate" :min="new Date(new Date().setDate(new Date().getDate() - 1))"
                no-title scrollable class="ml-8"></v-date-picker>
            </v-col>
            <v-col>
              <p class="text-h4 mt-12 pt-5" style="font-size: 32px !important">
                Select building
              </p>
              <v-select class="my-8 mr-5" label="Select" chips variant="outlined" :items="buildings" item-title="name"
                v-model="selectedBuilding" :hint="selectedBuilding !== null
  ...
  <v-row scrollable class="ml-12">
              ...
              <v-select 
                ...
                <template v-slot:item="{ props, item }">
                  <v-list-item v-bind="props" id="prettySelect">
                    <v-list-item-content>
                      <v-list-item-title>
                        <v-icon class="mr-2">mdi-home-floor-{{ item.raw.numberOfFloor }}</v-icon>Floor {{item.raw.numberOfFloor }}
                      </v-list-item-title>
                      <v-list-item-subtitle class="d-flex align-center">
            ...
            <v-col cols="12" md="8" class="d-flex align-center justify-center">
              <Transition enter-active-class="animate__animated animate__fadeIn"
                ...
                <div v-if="wantsWindow != null" class="d-flex align-center">
                  <v-icon @click="wantsWindow = null" color="red" style="position: absolute">mdi-close-circle</v-icon>
                </div>
              </Transition>
              <v-checkbox v-model="wantsWindow" label="I want a window" class="ml-6 d-flex align-center"></v-checkbox>
            </v-col>
          </v-row>
        </v-card>
      </Transition>
        ...
        <div v-if="isSvgVisible && selectedBuilding.name == 'Building1'">
          <Transition enter-active-class="animate__animated animate__fadeIn" appear>
            <Floor1B1 v-if="selectedFloor == 1" :floorInfo="selectedBuilding.floors[0]" :occupiedDesks="occupiedDesks"
              :userType="userType" :equipment="selectedEquipment" :hasWindow="wantsWindow" @deskClicked="bookingDesk"
    ...
    <v-col cols="12" md="5" class="my-12 mx-auto" justify="center" style="max-width: 400px">
      <Transition enter-active-class="animate__animated animate__fadeIn">
        <div v-if="deskDetails && isSvgVisible" class="desk-details">
          <v-card elevation="5" class="rounded-lg" v-if="deskDetails.type !== 'meeting_room'">
            <v-card-title class="headline">Desk Details</v-card-title>
            <v-card-text>
              <v-list>
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-title><strong>Name desk:</strong>
                      {{ deskDetails.name }}</v-list-item-title>
                  </v-list-item-content>
                </v-list-item>
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-title><strong>Type:</strong>
                ...
                >
                  <v-list-item-content>
                    <v-list-item-title><strong>Equipment:</strong>
                      {{ deskDetails.equipment }}</v-list-item-title>
                  </v-list-item-content>
                </v-list-item>
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-title><strong>Floor:</strong>
                      {{ selectedFloor }}</v-list-item-title>
                  </v-list-item-content>
                </v-list-item>
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-title><strong>Price:</strong>
                ...
                <v-list-item>
                  <p>Assign to:<br><br></p>
                  <v-select :items="employeeList" item-value="user.id" item-title="fullName" label="User"
                    v-model="booking.userId" :disabled="isAddClicked" variant="outlined">
                  </v-select>
                </v-list-item>
            ...
            <v-card-actions>
              <v-btn color="primary" @click="assignBookingToUser" :disabled="!booking.userId">Add</v-btn>
            </v-card-actions>
          </v-card>
          <v-card elevation="5" v-else>
            ...
            <v-card-text>
              <v-list>
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-title><strong>Name room:</strong>
                      {{ deskDetails.name }}</v-list-item-title>
                  </v-list-item-content>
                </v-list-item>
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-title><strong>Number of seats:</strong>
                ...
                <v-list-item>
                  <p>Assign to:<br><br></p>
                  <v-select class="mt-4" clearable label="Select" :items="employeeList" item-value="user"
                    item-title="fullName" v-model="meetingRoomList" chips multiple variant="outlined"
            ...
            <v-card-actions>
              <v-btn color="primary" @click="assignMeetingRoomToUsers"
                :disabled="meetingRoomList.length == 0 || isAddClicked">Add</v-btn>
            </v-card-actions>
          </v-card>
        </div>
      </Transition>
    ...
    <v-fade-transition hide-on-leave>
      <v-dialog v-model="alertVisible" persistent min-width="500" max-width="800">
        <v-card class="mx-auto booking-alert" elevation="16" min-width="500" max-width="800" :title="alertType === 'success' ? 'Confirmed booking' : (alertType === 'warning' ? 'Warning!' : 'Error booking')
      " :text="alertText" style="white-space: pre-line;">
          ...
          <div class="py-12 text-center" v-if="alertType !== 'warning'">
            <v-icon class="mb-6" size="100" :color="alertType === 'success' ? 'success' : 'error'">
              {{
                alertType === "success"
  ...
  <v-row class="d-flex justify-center align-center">
    <div class="mx-16" v-if="isSvgVisible && bookingList.length > 0">
      <v-data-table class="mx-auto my-16" :headers="headers" :items="bookingList"
        ...
        <template v-slot:item.actions="{ item }">
          <v-icon size="small" @click="deleteItem(item)" :class="{
      disabled:
...
export default {
  ...
  methods: {
      ...
      if (this.selectedDate !== null && this.selectedBuilding !== null) {
        ...
        this.$ApiService.find_occupied_desks(date).then((occupiedDesks) => {
          ...
          occupiedDesks.data.forEach((wsOccupied) => {
            if ((deskElement && wsOccupied.status !== "canceled") && (wsOccupied.user.company.id == this.companyId)) {
    ...
    assignMeetingRoomToUsers() {
      let users = this.savedUsersMeetingRoomList.find(room => room.meeting_room_name === this.deskDetails.name) ? this.savedUsersMeetingRoomList.find(room => room.meeting_room_name === this.deskDetails.name).users : [];
